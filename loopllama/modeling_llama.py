...
class LoopMergingStaticCache(StaticCache):
    """
    在相同绝对位置做就地融合（默认 EMA），用 gather/scatter_ 实现。
    通过 cache_kwargs 传参:
      - cache_position: LongTensor[S_step] 绝对位置
      - alpha: float, EMA 系数, 默认 0.5
      - merge: bool, True=融合, False=覆盖
    注：若单个 step 内 cache_position 含重复位置，建议先去重或分批处理。
    """

    def __init__(self, 
                *args, 
                default_alpha: float = 0.5, 
                duplicate_policy: str = "mean",   # 可选: "mean" / "last" / "sum"
                **kwargs):
        ...
        # 为每层建“是否已写过”的掩码: [B, T]
        self._written_mask = [
            torch.zeros(self.batch_size, self.max_cache_len, dtype=torch.bool, device=self.key_cache[i].device)
    ...
    def update(self, key_states, value_states, layer_idx, cache_kwargs=None):
        ...
        # ---- 2) 首写与再写分流（冷启动安全）----
        written = self._written_mask[layer_idx]            # [B, T]
        # 对每个 batch，取该批次的已写标记
        # fresh_mask: [B, S]（本次这些 pos 是否首次写入）
        fresh_mask = (~written[:, pos])                    # [B, S]

        # 为 gather/scatter 准备索引 [B,H,S,D]
    ...
    def forward(
        ...
        if use_cache:
            if past_key_values is None or not isinstance(past_key_values, StaticCache):
                # 统一转为 LoopMergingStaticCache（静态预分配）
                # max_cache_len = getattr(self.config, "max_position_embeddings", None)
                # if max_cache_len is None:
                #     # 兜底：用当前序列长度或 cache_position 最大值 + 预留
                #     max_cache_len = (
